...
![MiniFlex 整体架构示意图](./image/说明图/9605bfff8bf6bbf7a08bec89d7c41d7.png)
...
![长上下文场景下的加速比](./image/说明图/380ee73b3ea3129e081402a63e14fc2.png)
...
![超容量场景下 MiniFlex 与 APC 对比](./image/说明图/d09bfe6aece1d535730ca0a55bddab7.png)
...
![混合负载场景结果](./image/说明图/2af5f5a8a15123ca0de0e22ddc0f068.png)
...
团队整体采用“前期集中调研 + 中期并行开发 + 后期联合测试与材料整理”的协作方式推进项目。在开发过程中，成员围绕 vLLM 接入、缓存结构、传输链路、测试脚本和文档整理等任务分工协作，并通过 Git 提交、阶段性讨论和结果复核保持实现与文档的一致性。
...
本项目提交仓库主目录为 `project-21/`。仓库整体结构如下：
...
| 路径 | 说明 |
|---|---|
| `README.md` | 仓库入口说明，介绍项目背景、运行方式和主要文档索引。 |
| `设计开发文档.md` | 比赛主体设计开发文档，记录项目目标、方案设计、推进过程、测试情况、分工和收获。 |
| `miniflex/` | MiniFlex 项目核心目录，包含源码、测试脚本、演示脚本和工程配置。 |
| `miniflex/pysrc/` | Python 核心实现目录，包含缓存管理、存储管理、传输调度和 vLLM 接入代码。 |
| `miniflex/csrc/` | C++ / CUDA 扩展目录，包含 GPU↔CPU 传输和 SSD I/O 相关底层实现。 |
| `miniflex/docs/` | 项目说明文档目录，包括项目结构、使用说明、验证结果和测试整理说明。 |
| `miniflex/test/` | 单元测试和模块测试目录，用于验证 cache、storage、transfer、vLLM adapter 等模块。 |
| `归档-日志-调研-测试/` | 比赛过程归档目录，保存调研材料、测试材料和历史记录。 |

### 10.2 核心源码目录说明

| 路径 | 说明 |
|---|---|
| `miniflex/pysrc/miniflex/cache/` | 逻辑缓存相关实现，包括前缀树、内存池、全局缓存引擎等。 |
| `miniflex/pysrc/miniflex/storage/` | 物理存储相关实现，负责缓存块在不同存储层中的组织和管理。 |
| `miniflex/pysrc/miniflex/transfer/` | 传输调度相关实现，负责跨层数据搬运任务的组织和执行。 |
| `miniflex/pysrc/miniflex/integration/vllm/` | vLLM 接入层实现，包括 connector 和 V1 adapter。 |
| `miniflex/pysrc/miniflex/common/` | 公共数据结构和工具模块，包括 block、hash、request、storage、transfer 等定义。 |
| `miniflex/csrc/transfer.cu` | CUDA 传输相关底层实现。 |
| `miniflex/csrc/ssd_io_uring.cpp` | SSD I/O 相关底层实现。 |
| `miniflex/csrc/bindings.cpp` | C++ / CUDA 扩展与 Python 侧绑定入口。 |
...
4. **测试验证**：`bench_ttft.py`、`bench_overflow.py`、`bench_mixed.py`、`bench_full.py` 和 `validation.md` 对应系统测试情况；
5. **调研归档**：`Implementation_Investigation/` 中保存了方案设计前的技术调研材料；
6. **演示材料**：`demo.sh`、`run_vllm_miniflex.sh` 和 `MiniFlexdemo.mp4` 用于展示项目运行流程。
...
5. `miniflex/bench_ttft.py`、`miniflex/bench_overflow.py`、`miniflex/bench_mixed.py`：实验脚本；

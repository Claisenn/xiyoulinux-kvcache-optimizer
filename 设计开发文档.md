...
3. 验证 GPU↔CPU、CPU↔SSD 两类传输 Worker 与传输管理器的状态协同；
4. 验证 vLLM connector 接入层与 Worker 主流程在当前实现下能够稳定工作。

### 7.4 集成演示测试与 demo.sh 脚本说明

除单元测试外，项目还提供了面向录屏与答辩演示的端到端脚本 `miniflex/demo.sh`。该脚本不是单元测试入口，而是一个自包含的集成演示脚本：它会自动配置运行环境、启动 `vllm serve` 服务，并依次调用若干 benchmark 脚本完成多类场景验证。
...
视频录制中展示的是端到端演示和场景测试流程，而不是单元测试执行过程。
...
├── MiniFlexdemo.mp4                               # 项目演示视频
├── 初赛PPT.pptx                                    # 项目答辩与展示材料
├── image/                                         # 文档配图与过程截图
│   ├── PR图片/                                    # PR、协作记录相关截图
│   └── 说明图/                                    # 开发文档中的说明性图片
...
| `MiniFlexdemo.mp4` | 项目演示视频，用于展示系统运行与测试效果。 |
| `初赛PPT.pptx` | 项目答辩与展示材料。 |
...
| `miniflex/csrc/` | 项目底层 C++ / CUDA 扩展源码目录，包含 SSD I/O 和 GPU↔CPU 传输相关实现。 |
...
| `miniflex/pysrc/miniflex/transfer/transfer_engine.py` | 传输引擎实现，负责组织 GPU↔CPU、CPU↔SSD 的搬运任务。 |
...
| `miniflex/csrc/` | 底层 C++ / CUDA 扩展源码目录，负责高性能 SSD I/O 与 GPU↔CPU 传输支持。 |
